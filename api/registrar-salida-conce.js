...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!record) {
      return res.status(404).json({ error: 'No se encontró un registro activo de Conce para cerrar.' });
    }
    ...
    if (!registroActualizado) {
      return res.status(409).json({ error: 'No se pudo cerrar el registro activo de Conce.' });
    }
