...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const requestedCampus = String(body.campus || '').trim();
    const campus = FIXED_CAMPUS;
    ...
    if (requestedCampus && requestedCampus !== FIXED_CAMPUS) {
      return res.status(400).json({ error: 'El campus para esta ruta debe ser Conce.' });
    }
