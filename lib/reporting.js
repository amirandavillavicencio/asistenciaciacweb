...
function buildAttendanceRanking(records, limit = 10) {
  const safeRecords = Array.isArray(records) ? records : [];
  const byRun = safeRecords.reduce((acc, record = {}) => {
    const run = String(record.run || '').replace(/\D/g, '');
    if (!run) return acc;

    const key = `${run}-${String(record.dv || '').toUpperCase()}`;
    if (!acc[key]) {
      acc[key] = {
        run,
        dv: String(record.dv || '').toUpperCase(),
        student: String(record.nombre || record.estudiante || '').trim(),
        carrera: normalizeLabel(record.carrera, 'Sin carrera'),
        totalAsistencias: 0,
      };
    }

    acc[key].totalAsistencias += 1;

    if (!acc[key].student) {
      const candidate = String(record.nombre || record.estudiante || '').trim();
      if (candidate) acc[key].student = candidate;
    }

    if (!acc[key].carrera || acc[key].carrera === 'Sin carrera') {
      acc[key].carrera = normalizeLabel(record.carrera, 'Sin carrera');
    }
  ...
  return Object.values(byRun)
    .sort((a, b) => b.totalAsistencias - a.totalAsistencias || a.run.localeCompare(b.run, 'es-CL'))
    .slice(0, Math.max(1, Number(limit) || 10))
    .map((row, index) => ({
      position: index + 1,
      student: row.student || 'Sin nombre',
      run: row.run,
      dv: row.dv || '',
      rut: formatRut(row.run, row.dv),
      carrera: row.carrera,
      totalAsistencias: row.totalAsistencias,
    }));
}
