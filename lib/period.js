...
const { normalizeRunFromInput } = require('./rut');
...
function applyCommonFilters(query, reqQuery = {}) {
  ...
  const rut = String(reqQuery.rut || reqQuery.run || '').trim();
  const run = normalizeRunFromInput(rut);

  if (campus) query.sede = `eq.${campus}`;
  if (actividad) query.actividad = `eq.${actividad}`;
  if (tematica) query.tematica = `eq.${tematica}`;
  if (run) query.run = `eq.${run}`;
  return { campus, actividad, tematica, rut, run };
}
